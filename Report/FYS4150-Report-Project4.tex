...
For $T = 1.0$ kT/J we find that $P(E) = 0.87$ and $\sigma_E^2 = 0.02$, while for $T = 2.4$ kT/J we find that $P(E) = 0.03$ and $\sigma_E^2 = 8.28$ in the case of an ordered initial spin matrix. This means that where there is a small deviation in energy values, described by the variance, there will be a high probability of getting the specific value $E$. When the deviation is large as for $T=2.4$ kT/J, the probability of getting the value $E$ is small.

% (according to Fig.~\ref{fig:random} this result would not be different for a randomly oriented spin matrix because we are excluding the first number of Monte Carlo cycles where having a different initial configuration would make a difference)
