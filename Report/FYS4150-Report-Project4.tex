...
The critical temperature $T_{\mathrm{C}}$ is described by Eq. (\ref{eq:T_C}), where $T_{\mathrm{C}}(L)$ is calculated for different lattice sizes $L$ through our algorithm described above.  $T_{\mathrm{C}}(L = \infty)$ is described by the exact value of 2.269 (in units of $kT/J$). The equation has two unknowns, $T_{\mathrm{C}}(L = \infty)$ and the constant $a$. By looking at our calculations for two different lattice sizes, we can solve Eq. (\ref{eq:T_C}) in the two cases and get an estimate for $a$ and thus the critical temperature $T_C$. 
